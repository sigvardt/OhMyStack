...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/ohmystack-builder-profile`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ohmystack-profile-'));
});
...
describe('ohmystack-builder-profile', () => {
