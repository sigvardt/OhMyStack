...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // ohmystack-review-read reads from ~/.ohmystack/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock ohmystack-review-read script that returns our test data
    ...
    // Write a mock ohmystack-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'ohmystack-review-read'), [
      '#!/usr/bin/env bash',
    ...
    fs.chmodSync(path.join(mockBinDir, 'ohmystack-review-read'), 0o755);
...
Instead of running ~/.claude/skills/ohmystack/bin/ohmystack-review-read, run this mock: ${mockBinDir}/ohmystack-review-read
