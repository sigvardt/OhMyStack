...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['ohmystack-learnings-search', 'ohmystack-learnings-log', 'ohmystack-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what ohmystack-slug computes.
    // With no git remote, ohmystack-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/ohmystack/bin/ohmystack-slug with ./bin/ohmystack-slug.
