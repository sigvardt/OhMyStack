 ...
 * ~/.ohmystack/projects/$SLUG/evals/{version}-{branch}-{tier}-{timestamp}.json,
...
const LEGACY_EVAL_DIR = path.join(os.homedir(), '.ohmystack-dev', 'evals');

/**
 * Detect project-scoped eval dir via ohmystack-slug.
 * Falls back to legacy ~/.ohmystack-dev/evals/ if slug detection fails.
 */
export function getProjectEvalDir(): string {
  try {
    // Try repo-local ohmystack-slug first, then global install
    const localSlug = spawnSync('bash', ['-c', '.claude/skills/ohmystack/bin/ohmystack-slug 2>/dev/null || ~/.claude/skills/ohmystack/bin/ohmystack-slug 2>/dev/null'], {
    ...
    if (output) {
      ...
      if (slugMatch && slugMatch[1]) {
        const dir = path.join(os.homedir(), '.ohmystack', 'projects', slugMatch[1], 'evals');
