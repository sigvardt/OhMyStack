...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
4. Launch background explore agents before the rest of pre-flight so you have extra context while the main flow continues:

   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Pre-flight for /ship on branch <branch-name> against <base>. Find uncommitted or adjacent related changes that should likely ship together, and flag risky omissions or hidden scope edges.")
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Pre-flight for /ship on branch <branch-name> against <base>. Inspect changed files and existing tests to find likely coverage gaps, weak assertions, or code paths that may be under-tested.")
   ```

   Do **not** block on these yet. Let them run while Steps 1.5-3.4 execute. Fold their findings into the pre-flight summary, coverage audit, and review gate when the results are ready.
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`ohmystack-config set skip_eng_review true\` (the "don't bother me" setting).
...
For Design Review: run `source <(~/.opencode/skills/ohmystack/bin/ohmystack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f .ohmystack/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `.ohmystack/no-test-bootstrap` and continue without tests.
...
If user picks C → write `.ohmystack/no-test-bootstrap`. Tell user: "If you change your mind later, delete `.ohmystack/no-test-bootstrap` and re-run." Continue without tests.
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** OhMyStack /ship on <date>" \
...
```bash
eval "$(~/.config/opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.ohmystack/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for ~/.ohmystack/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.ohmystack/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".ohmystack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## OHMYSTACK REVIEW REPORT`)
...
```bash
_CROSS_PROJ=$($OHMYSTACK_BIN/ohmystack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $OHMYSTACK_BIN/ohmystack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
...
If A: run `$OHMYSTACK_BIN/ohmystack-config set cross_project_learnings true`
If B: run `$OHMYSTACK_BIN/ohmystack-config set cross_project_learnings false`
...
3. Before applying the checklist, launch the hard review gate in parallel using the diff summary from Step 2:

   ```typescript
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Review this diff for: SQL safety, trust boundary violations, conditional side effects. Diff: [diff summary]")
   task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="Adversarial critique of this diff: [diff summary]. Find gaps, missing edge cases, untested paths.")
   ```

   Wait for both review agents before Step 8. Both must pass for PR creation to proceed. If either agent finds blocking risk, treat it like a Step 3.5 ASK/BLOCKER outcome and resolve it before continuing.

4. Apply the review checklist in two passes:
...
Check if the diff touches frontend files using `ohmystack-diff-scope`:

```bash
source <($OHMYSTACK_BIN/ohmystack-diff-scope <base> 2>/dev/null)
```
...
```bash
$OHMYSTACK_BIN/ohmystack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($OHMYSTACK_BIN/ohmystack-diff-scope <base> 2>/dev/null) || true
...
```bash
$OHMYSTACK_BIN/ohmystack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$OHMYSTACK_ROOT/review/specialists/testing.md`
2. **Maintainability** — read `$OHMYSTACK_ROOT/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$OHMYSTACK_ROOT/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$OHMYSTACK_ROOT/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$OHMYSTACK_ROOT/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$OHMYSTACK_ROOT/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$OHMYSTACK_ROOT/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `ohmystack-specialist-stats` output above:
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$OHMYSTACK_ROOT/review/specialists/red-team.md`
...
5. **Classify each finding from the checklist pass, specialist review (Step 3.55-3.56), and the Oracle/Momus gate as AUTO-FIX or ASK** per the Fix-First Heuristic in
   checklist.md. Critical findings lean toward ASK; informational lean toward AUTO-FIX.

6. **Auto-fix all AUTO-FIX items.** Apply each fix. Output one line per fix:
...
10. Persist the review result to the review log:
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
Include a one-line Oracle/Momus gate verdict in the saved review output so the PR body makes it explicit that both parallel review agents passed before PR creation.
...
OLD_CFG=$(~/.config/opencode/skills/ohmystack/bin/ohmystack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.config/opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- Message: "OhMyStack recommends maintaining a TODOS.md organized by skill/component, then priority (P0 at top through P4, then Completed at bottom). See TODOS-format.md for the full format. Would you like to create one?"
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
```

Append to `~/.ohmystack/projects/$SLUG/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.ohmystack/projects/$SLUG/$BRANCH-reviews.jsonl
```
