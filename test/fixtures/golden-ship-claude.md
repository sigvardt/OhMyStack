...
~/.claude/skills/ohmystack/bin/ohmystack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
eval "$(~/.claude/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
...
Before building anything unfamiliar, **search first.** See `~/.claude/skills/ohmystack/ETHOS.md`.
...
```bash
eval "$(~/.claude/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
USER=$(whoami)
...
```bash
_CROSS_PROJ=$(~/.claude/skills/ohmystack/bin/ohmystack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/ohmystack/bin/ohmystack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.claude/skills/ohmystack/bin/ohmystack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `~/.claude/skills/ohmystack/bin/ohmystack-config set cross_project_learnings true`
If B: run `~/.claude/skills/ohmystack/bin/ohmystack-config set cross_project_learnings false`
...
```bash
source <(~/.claude/skills/ohmystack/bin/ohmystack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.claude/skills/ohmystack/bin/ohmystack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.claude/skills/ohmystack/review/specialists/testing.md`
2. **Maintainability** — read `~/.claude/skills/ohmystack/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.claude/skills/ohmystack/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.claude/skills/ohmystack/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.claude/skills/ohmystack/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.claude/skills/ohmystack/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.claude/skills/ohmystack/review/design-checklist.md`
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.claude/skills/ohmystack/review/specialists/red-team.md`
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$(~/.claude/skills/ohmystack/bin/ohmystack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.claude/skills/ohmystack/bin/ohmystack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
