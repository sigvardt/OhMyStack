...
const scriptPath = join(import.meta.dir, "..", "bin", "ohmystack-global-discover.ts");

describe("ohmystack-global-discover", () => {
  describe("normalizeRemoteUrl", () => {
    // Dynamically import to test the exported function
    let normalizeRemoteUrl: (url: string) => string;

    beforeEach(async () => {
      const mod = await import("../bin/ohmystack-global-discover.ts");
    ...
    test("SSH and HTTPS for same repo normalize to same URL", () => {
      const ssh = normalizeRemoteUrl("git@github.com:sigvardt/OhMyStack.git");
      const https = normalizeRemoteUrl("https://github.com/sigvardt/OhMyStack.git");
      const httpsNoDotGit = normalizeRemoteUrl("https://github.com/sigvardt/OhMyStack");
  ...
  describe("codex large session_meta parsing", () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), "ohmystack-codex-test-"));
