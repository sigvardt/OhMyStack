...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/ohmystack-cli-');
    const execPath = path.join(root, '.claude/skills/ohmystack/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/ohmystack/browse/src/server.ts');
