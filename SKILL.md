...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ohmystack","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
If `PROACTIVE` is `false`: do NOT proactively invoke or suggest other ohmystack skills during
this session. Only run skills the user explicitly invokes. This preference persists across
sessions via `ohmystack-config`.
...
- User asks to upgrade ohmystack → invoke `/ohmystack-upgrade`
...
If the user opts out of suggestions, run `ohmystack-config set proactive false`.
If they opt back in, run `ohmystack-config set proactive true`.

# ohmystack browse: QA Testing & Dogfooding
