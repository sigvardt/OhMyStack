...
description: |
  ...
  Proactively suggest when the user wants a bug report without any code changes. (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa-only","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"qa-only","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
| Mode | full | `--quick`, `--regression .ohmystack/qa-reports/baseline.json` |
...
```bash
REPORT_DIR=".ohmystack/qa-reports"
...
## Multi-Agent Dispatch Overlay

Before testing begins, dispatch mapping agents so report-only QA covers the real surface area while still honoring the "never fixes anything" constraint:

```python
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="TASK: Map all user-facing test surfaces for report-only QA. Find routes/pages, auth-gated flows, forms, nav entry points, settings pages, empty states, and destructive actions. Return a browse-oriented checklist only; do not suggest fixes.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="TASK: Enumerate interactive elements that deserve QA coverage. Find form submissions, modals, dropdowns, uploads, filters, pagination, inline validation, and error states. Return repro-focused notes with labels/selectors when available.")
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="TASK: Severity-classify QA findings. For each reported issue, use the evidence provided to classify severity, user impact, confidence, and whether it should block ship. Do not propose code fixes.")
```

Use the explore results to plan browse coverage, then use Oracle after issues are found to normalize severity scoring in the structured report. The agents support decision quality; they do NOT replace the real testing workflow.
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"qa-only","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
13. **Dispatch is analysis-only.** Explore may map surfaces and Oracle may classify severity, but neither agent may be used to fix, patch, or rewrite code in `/qa-only`.
