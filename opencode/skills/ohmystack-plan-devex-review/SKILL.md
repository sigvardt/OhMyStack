...
description: |
  ...
  (APIs, CLIs, SDKs, libraries, platforms, docs). (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-devex-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
\`$OHMYSTACK_ROOT/plan-devex-review/dx-hall-of-fame.md\`
...
### OMO Multi-Agent DX Dispatch

Before Step 0, convert DX research into explicit multi-agent dispatch. Keep the audit reads above;
use sub-agents to deepen the persona, benchmark, and strategy work.

**Phase 1: DX evidence gathering (parallel)**

1. **Explore — developer persona signals in the repo**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Developer-experience plan review for [feature/topic]. [GOAL] Infer the likely target developer personas from the existing repo. [REQUEST] Search README/docs/examples/package metadata/CLI help for audience clues, assumptions about developer skill level, install expectations, and time-to-hello-world signals. Return evidence-backed persona candidates.")
   ```

2. **Explore — current DX friction inventory**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] DX review for [feature/topic]. [GOAL] Find current friction points in the developer journey before I score the plan. [REQUEST] Search docs, examples, setup scripts, error messages, CLI help text, and changelog/review notes for confusing steps, missing prerequisites, inconsistent language, and signs of onboarding pain.")
   ```

3. **Librarian — competitor DX benchmarking**
   ```typescript
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT] Reviewing the DX plan for [feature/topic]. [GOAL] Benchmark the product against comparable developer tools. [REQUEST] Research close competitors and adjacent best-in-class tools, focusing on time-to-hello-world, onboarding flow, magical-moment delivery, error quality, docs structure, and self-serve activation patterns.")
   ```

Wait for the background agents before persona confirmation and competitive scoring. Use their findings in 0A, 0B, 0C, and 0F.

**Phase 2: DX strategy evaluation (sync)**

4. **Oracle — strategy pressure test**
   ```typescript
   task(category="deep", load_skills=[], run_in_background=false, prompt="[CONTEXT] DX plan review for [feature/topic]. I have internal persona evidence, a friction inventory, and external benchmark research. [GOAL] Validate the DX strategy before I guide the user through mode selection and scoring. [REQUEST] Evaluate whether this plan should target DX EXPANSION, DX POLISH, or DX TRIAGE; identify the highest-leverage magical moment; and name the most dangerous adoption risk if the current plan ships unchanged.")
   ```
...
Load the "## Pass 1" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
~/.opencode/skills/ohmystack/bin/ohmystack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 2" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 3" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
This is NOT a scored pass. It's a checklist of proven patterns from ohmystack's own DX.

Load reference: Read the "## Claude Code Skill DX Checklist" section from
`~/.opencode/skills/ohmystack/plan-devex-review/dx-hall-of-fame.md`.
...
`~/.ohmystack/` (user config directory, not project files).

```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"TIER","pass_scores":{"getting_started":N,"api_design":N,"errors":N,"docs":N,"upgrade":N,"dev_env":N,"community":N,"measurement":N},"unresolved":N,"commit":"COMMIT"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
