...
description: |
  ...
  "watch production", "verify deploy". (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"canary","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"canary","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
mkdir -p .ohmystack/canary-reports
mkdir -p .ohmystack/canary-reports/baselines
mkdir -p .ohmystack/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".ohmystack/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.ohmystack/canary-reports/baseline.json`:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".ohmystack/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".ohmystack/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```

Launch parallel monitoring agents so different health dimensions are watched independently throughout the canary window:

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Monitor console errors on [URL]. Take periodic screenshots. Report anomalies.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Monitor performance metrics on [URL]. Compare against pre-deploy baseline.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Monitor screenshot/content drift and broken-page symptoms on [URL] across the selected pages. Report visual regressions, blank states, and new 404-like behavior.")
```

Let these run in parallel with the foreground loop. Aggregate their evidence into the per-check alert evaluation rather than duplicating the same manual observations.
...
When an alert persists or the evidence is ambiguous, launch Oracle before presenting the recommendation:

```typescript
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Analyze this canary anomaly set for [URL]: [alerts, screenshots, console output, perf deltas]. Determine severity, likely root cause class, and whether the issue looks transient or deploy-induced.")
```

Use the Oracle result to calibrate the recommendation and distinguish real regressions from noisy warm-up effects.
...
Screenshots:   .ohmystack/canary-reports/screenshots/

VERDICT: [DEPLOY IS HEALTHY / DEPLOY HAS ISSUES — details above]
```

Save report to `.ohmystack/canary-reports/{date}-canary.md` and `.ohmystack/canary-reports/{date}-canary.json`.
