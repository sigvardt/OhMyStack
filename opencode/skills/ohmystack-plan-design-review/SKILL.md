...
description: |
  ...
  should be reviewed before implementation. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
### The ohmystack designer — YOUR PRIMARY TOOL

You have the **ohmystack designer**, an AI mockup generator that creates real visual mockups
...
### OMO Multi-Agent Design Dispatch

Before Step 0, replace generic "find existing patterns" work with an explicit
design-focused agent swarm. These agents gather evidence; they do not replace the
0-10 scoring loop or the mockup workflow.

**Phase 1: Existing-pattern research (parallel)**

1. **Explore — current design system + component patterns**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Plan design review for [feature/topic]. [GOAL] Find existing UI patterns in the repo that this plan should align with. [REQUEST] Search for reusable components, layout patterns, interaction states, responsive behaviors, design tokens, and DESIGN.md references. Return concrete components, file paths, and anti-patterns to avoid.")
   ```

2. **Explore — user-flow and edge-case precedents**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Designer's-eye review for [feature/topic]. [GOAL] Find how the current product already handles loading, empty, error, success, onboarding, and mobile states in adjacent flows. [REQUEST] Search frontend files, docs, and prior plans for state coverage patterns and call out gaps the new plan must not repeat.")
   ```

3. **Momus — adversarial critique of the design direction**
   ```typescript
   task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="[CONTEXT] Reviewing the design plan for [feature/topic]. [GOAL] Attack the plan's design decisions before implementation starts. [REQUEST] Critique the information hierarchy, user journey, AI-slop risk, responsiveness, accessibility, and trust signals. Be adversarial: point out where the plan sounds polished but would still produce a generic or confusing experience.")
   ```

Wait for the background agents before Step 0A and Step 0C. Use the findings to justify each 0-10 rating, define what would make the plan a 10, and focus mockup generation on the highest-risk gaps.
...
MUST be saved to `~/.ohmystack/projects/$SLUG/designs/`, NEVER to `.context/`,
...
ohmystack designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the ohmystack designer. This is how we
...
`~/.ohmystack/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The ohmystack
...
- `mkdir -p ~/.ohmystack/projects/$SLUG/designs/...`
...
_DESIGN_DIR=~/.ohmystack/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The ohmystack designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
**Log the result:**
```bash
$OHMYSTACK_BIN/ohmystack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
When the OMO agent swarm above returns findings, cite them in the rating rationale. Example:
- "Information Architecture is 4/10 because the explore agent found three adjacent flows with stronger hierarchy than this plan specifies."
- "AI Slop Risk is 3/10 because the momus critique surfaced generic SaaS patterns without a distinctive interaction thesis."
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/ohmystack-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + OhMyStack design methodology.
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.ohmystack/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
