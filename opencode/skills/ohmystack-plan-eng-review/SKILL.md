...
description: |
  ...
  start coding — to catch architecture issues before implementation. (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-eng-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"plan-eng-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
### OMO Multi-Agent Architecture Dispatch

Before Step 0, replace vague "analyze the architecture" work with an explicit agent swarm.
Keep the design doc check and normal file reads above; use agents for the deeper pattern scan.

**Phase 1: Codebase and best-practice scan (parallel)**

1. **Explore — existing architecture patterns**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Engineering plan review for [feature/topic]. [GOAL] Find the architecture patterns already used in this repo that are closest to the proposed plan. [REQUEST] Search for similar services, controllers, jobs, data flows, dependency boundaries, and rollout patterns. Return reusable modules, coupling risks, and examples with file paths.")
   ```

2. **Explore — operational hotspots and review history**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Eng review for [feature/topic]. [GOAL] Find the parts of the codebase most likely to create blast radius if this plan touches them. [REQUEST] Search recent history, TODO/FIXME clusters, prior review notes, integration seams, test hotspots, and modules with recurring churn. Summarize likely landmines and what already looks fragile.")
   ```

3. **Librarian — external best practices on proposed tech choices**
   ```typescript
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT] Reviewing a proposed technical approach for [feature/topic]. [GOAL] Gather current best practices and known footguns for the tech choices in this plan. [REQUEST] Research framework built-ins, recommended architecture patterns, scaling/operability guidance, and common mistakes for the proposed concurrency/infrastructure/data-access choices.")
   ```

Wait for the background agents before Step 0 findings. Feed their output into scope challenge, complexity checks, and production-failure analysis.

**Phase 2: Architecture tradeoff consultation (sync)**

4. **Oracle — architecture tradeoff analysis**
   ```typescript
   task(category="deep", load_skills=[], run_in_background=false, prompt="[CONTEXT] Engineering plan review for [feature/topic]. I have repo pattern scans plus external best-practice research. [GOAL] Pressure-test the proposed architecture before I walk the user through Step 0 and Section 1. [REQUEST] Compare the likely implementation approaches, identify the best boring-default path, flag any innovation-token spending, and name the highest-risk tradeoffs I should surface to the user.")
   ```
...
Write to `~/.ohmystack/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
