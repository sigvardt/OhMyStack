...
description: |
  ...
  Proactively suggest at the end of a work week or sprint. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Before running the local git commands, launch parallel background researchers so the retro has higher-level synthesis ready by the time the raw data is computed:

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Weekly retro research for window <window>. Analyze commit history shape, release cadence, notable clusters, and inflection points from the repo history.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Weekly retro research for window <window>. Analyze work patterns: session clustering, hotspots, test discipline shifts, and which contributors appear most focused vs fragmented.")
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Benchmark this team's recent engineering velocity and quality signals against industry norms for small high-output software teams. Focus on commit cadence, PR size, test ratio, and sustained shipping pace.")
```

Let these run while Step 1 gathers raw data. When the results return, fold them into the summary, team-aware breakdown, praise/growth areas, and trend commentary. The librarian benchmark is context, not a grading rubric — use it to sharpen the narrative, not flatten project-specific nuance.
...
cat ~/.ohmystack/greptile-history.md 2>/dev/null || true
...
# 12. ohmystack skill usage telemetry (if available)
cat ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Skill Usage (if analytics exist):** Read `~/.ohmystack/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `~/.ohmystack/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `~/.ohmystack/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
cat ~/.ohmystack/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x ~/.opencode/skills/ohmystack/bin/ohmystack-global-discover ] && DISCOVER_BIN=~/.opencode/skills/ohmystack/bin/ohmystack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .opencode/skills/ohmystack/bin/ohmystack-global-discover ] && DISCOVER_BIN=.opencode/skills/ohmystack/bin/ohmystack-global-discover
[ -z "$DISCOVER_BIN" ] && which ohmystack-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which ohmystack-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/ohmystack-global-discover.ts ] && DISCOVER_BIN="bun run bin/ohmystack-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the ohmystack directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/ohmystack-discover-stderr
```

Read the stderr output from `/tmp/ohmystack-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: ohmystack-global-discover" + "feat: /retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.ohmystack/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.ohmystack/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.ohmystack/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: ohmystack (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.ohmystack/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
