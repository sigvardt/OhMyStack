---
name: open-ohmystack-browser
description: |
  Launch OhMyStack Browser — AI-controlled Chromium with the sidebar extension baked in.
  Opens a visible browser window where you can watch every action in real time.
  The sidebar shows a live activity feed and chat. Anti-bot stealth built in.
  Use when asked to "open ohmystack browser", "launch browser", "connect chrome",
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-ohmystack-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"open-ohmystack-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
# /open-ohmystack-browser — Launch OhMyStack Browser

Launch OhMyStack Browser — AI-controlled Chromium with the sidebar extension,
...
if [ -f "$(git rev-parse --show-toplevel 2>/dev/null)/.ohmystack/browse.json" ]; then
  _OLD_PID=$(cat "$(git rev-parse --show-toplevel)/.ohmystack/browse.json" 2>/dev/null | grep -o '"pid":[0-9]*' | grep -o '[0-9]*')
  [ -n "$_OLD_PID" ] && kill "$_OLD_PID" 2>/dev/null || true
  sleep 1
  [ -n "$_OLD_PID" ] && kill -9 "$_OLD_PID" 2>/dev/null || true
  rm -f "$(git rev-parse --show-toplevel)/.ohmystack/browse.json"
fi
# Clean Chromium profile locks (can persist after crashes)
_PROFILE_DIR="$HOME/.ohmystack/chromium-profile"
...
This launches OhMyStack Browser (rebranded Chromium) in headed mode with:
- A visible window you can watch (not your regular Chrome — it stays untouched)
- The ohmystack sidebar extension auto-loaded via `launchPersistentContext`
- Anti-bot stealth patches (sites like Google and NYTimes work without captchas)
- Custom user agent and OhMyStack Browser branding in Dock/menu bar
- A sidebar agent process for chat commands

The `connect` command auto-discovers the extension from the ohmystack install
...
```bash
cat "$(git rev-parse --show-toplevel 2>/dev/null)/.ohmystack/browse.json" 2>/dev/null | grep -o '"port":[0-9]*' | grep -o '[0-9]*'
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.opencode/skills/ohmystack/extension/manifest.json" ] && _EXT_PATH="$_ROOT/.opencode/skills/ohmystack/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.opencode/skills/ohmystack/extension/manifest.json" ] && _EXT_PATH="$HOME/.opencode/skills/ohmystack/extension"
...
> Chrome is launched with ohmystack control. You should see Playwright's Chromium
...
>    already show the ohmystack icon if the extension loaded successfully
> 2. Click the **puzzle piece** → find **ohmystack browse** → click the **pin icon**
> 3. Click the pinned **ohmystack icon** in the toolbar
...
> 2. Look for **"ohmystack browse"** — it should be listed and enabled
...
>
> If the Side Panel badge stays gray (disconnected), click the ohmystack icon
...
> - Run any ohmystack skill (`/qa`, `/design-review`, `/benchmark`) and watch
