...
description: |
  ...
  onboarding". Proactively suggest after shipping a developer-facing feature. (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"devex-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Before Step 1, dispatch agents to map the DX surface and gather external context:

```python
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="TASK: Map developer touchpoints for this product. Find docs entry points, getting-started paths, install commands, quickstarts, CLI commands, examples, API playgrounds, auth/setup gates, error pages, CHANGELOGs, migration guides, contributing docs, and feedback channels. Return a testable DX checklist with file/URL evidence.")
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="TASK: Benchmark comparable developer tools. Find onboarding/getting-started expectations, time-to-hello-world ranges, and notable DX patterns for similar products. Return concise competitor baselines and links/evidence.")
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="TASK: Prepare a DX evaluation frame for this audit. Based on the product type, identify the highest-leverage DX dimensions, what good looks like, and what would count as a strategically serious DX gap.")
```

Use these agent outputs to shape the live audit. They improve prioritization and calibration; they do NOT replace real browse/bash testing.
...
~/.opencode/skills/ohmystack/bin/ohmystack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_PLAN_REVIEW"
```
...
Use the explore mapping to decide which getting-started flows must be timed. Measure TTHW from the actual first developer action to the first successful observable outcome.
...
Use the librarian benchmark to compare help-text quality, install friction, and expected onboarding time against similar tools instead of scoring in a vacuum.
...
After drafting the scorecard, consult Oracle for strategic synthesis before writing recommendations:

```python
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="TASK: Review this DX audit for strategic significance. SCORECARD: [scores]. TTHW: [measured]. COMPETITOR BASELINES: [librarian summary]. BOOMERANG DELTAS: [plan vs reality]. Return the top DX risks, strongest recommendations, and which problems most threaten adoption.")
```
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"devex-review","timestamp":"TIMESTAMP","status":"STATUS","overall_score":N,"product_type":"TYPE","tthw_measured":"TTHW","dimensions_tested":N,"dimensions_inferred":N,"boomerang":"YES_OR_NO","commit":"COMMIT"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
