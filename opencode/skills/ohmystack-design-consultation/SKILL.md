...
description: |
  ...
  design system or DESIGN.md. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"design-consultation","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)"
ls ~/.ohmystack/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the ohmystack designer (optional — enables AI mockup generation):**
...
### OMO multi-agent dispatch plan

As soon as product context is clear enough, use specialists to parallelize the heavy research and coherence checks:

1. **Librarian landscape research**
   ```
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Research the relevant design landscape for this product category. Focus on competitor design systems, typography trends, color system patterns, interaction tone, and any visual clichés to avoid. Return concrete references and a concise synthesis.")
   ```

2. **Explore codebase pattern scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Inspect the repo for existing design patterns, color tokens, typography choices, spacing scales, reusable components, marketing/app shells, and any current design drift. Return concrete file paths, token names, and patterns worth preserving or replacing.")
   ```

3. **Oracle coherence verifier**
   ```
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="After the initial design-system proposal is drafted, verify that the aesthetic, typography, color, spacing, layout, and motion choices form a coherent system for the stated product and audience. Call out contradictions, fragile choices, and any mismatch between the proposal and the codebase constraints.")
   ```

Use these dispatches to inform Phases 2-5. The main skill still owns the proposal, DESIGN.md output, and preview generation.
...
Kick off the Librarian and Explore dispatches at the start of this phase. Continue the user conversation while they run; do not duplicate their searches manually.
...
When specialist results land, blend them explicitly:
- **Librarian** informs the external landscape, category norms, typography trends, and color-theory opportunities.
- **Explore** informs what the codebase already supports and which tokens/components should anchor the recommendation.
- **Oracle** does not decide the taste for you; it verifies the proposal is internally coherent before you lock the system.
...
Before finalizing Q2, compare the proposal against:

1. the Librarian research synthesis,
2. the Explore codebase scan,
3. the Oracle coherence verification.

If Oracle flags a contradiction, keep the proposal conversational but surface the mismatch plainly: what clashes, why it matters, and whether you recommend adjusting it before generating previews or writing DESIGN.md.
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the ohmystack designer is available.
...
_DESIGN_DIR=~/.ohmystack/projects/$SLUG/designs/design-system-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"design-consultation","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
