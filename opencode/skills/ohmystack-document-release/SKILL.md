...
description: |
  ...
  Proactively suggest after a PR is merged or code is shipped. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"document-release","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"document-release","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
find . -maxdepth 2 -name "*.md" -not -path "./.git/*" -not -path "./node_modules/*" -not -path "./.ohmystack/*" -not -path "./.context/*" | sort
```
...
5. Launch an explore background scan before you start reading doc files manually:

   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Post-ship documentation scan for branch <branch-name> against <base>. Find every doc file that likely needs updating, including stale references, outdated paths, old command names, version mismatches, and docs contradicted by the shipped diff.")
   ```

   Let this run while you audit the obvious entry-point docs. Use its findings to widen the doc review set, catch stale references, and avoid missing secondary docs.
...
(adapt to whatever project you're in — these are not ohmystack-specific):
...
Dispatch the implementation work one doc at a time so each factual update is tightly scoped and easy to verify:

```typescript
For each doc needing update:
task(category="quick", load_skills=[], run_in_background=false, prompt="Update [doc-file] to match shipped changes: [specific changes needed]")
```

Use the main session to coordinate file selection, decide what changed, and review the results. Each quick task should target exactly one doc file with concrete requested deltas.
...
Before the final summary, run an Oracle verification pass on the docs you touched:

```typescript
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Verify that the updated documentation matches what actually shipped on branch <branch-name> against <base>. Check for factual drift, missing doc updates, stale references, and contradictions across README/ARCHITECTURE/CONTRIBUTING/CLAUDE/CHANGELOG/TODOS.")
```

Wait for the Oracle result before finishing. If Oracle finds factual mismatches, fix them before declaring the docs synced.
...
**If GitHub:**
```bash
gh pr view --json body -q .body > /tmp/ohmystack-pr-body-$$.md
```

**If GitLab:**
```bash
glab mr view -F json 2>/dev/null | python3 -c "import sys,json; print(json.load(sys.stdin).get('description',''))" > /tmp/ohmystack-pr-body-$$.md
```
...
**If GitHub:**
```bash
gh pr edit --body-file /tmp/ohmystack-pr-body-$$.md
```

**If GitLab:**
Read the contents of `/tmp/ohmystack-pr-body-$$.md` using the Read tool, then pass it to `glab mr update` using a heredoc to avoid shell metacharacter issues:
```bash
