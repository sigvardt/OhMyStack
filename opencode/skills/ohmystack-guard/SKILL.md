...
description: |
  ...
  "full safety", "lock it down", or "maximum safety". (ohmystack)
---
...
by the ohmystack setup script).

```bash
mkdir -p ~/.ohmystack/analytics
echo '{"skill":"guard","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
```
