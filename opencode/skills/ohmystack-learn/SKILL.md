...
name: learn
description: |
  Manage project learnings. Review, search, prune, and export what ohmystack
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
see what ohmystack has learned across sessions on this project, search for relevant
...
~/.opencode/skills/ohmystack/bin/ohmystack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
ohmystack will automatically capture patterns, pitfalls, and insights it discovers."

**OMO multi-agent dispatch (recommended when the recent list is noisy or dense):**

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn show recent. [GOAL]: Find the strongest repeated patterns across recent learnings. [REQUEST]: Search the latest learning entries, cluster repeated keys/themes, and summarize the highest-signal patterns.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn show recent. [GOAL]: Cross-check learnings against recent project history. [REQUEST]: Search project files, recent commits, and referenced paths to see which recent learnings still align with the current codebase.")
```

Use these agent summaries to improve grouping and call out which learnings are repeated, stale, or newly emerging.
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
~/.opencode/skills/ohmystack/bin/ohmystack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```

Replace USER_QUERY with the user's search terms. Present results clearly.

When the query is broad, ambiguous, or asks for "best"/"recommended" approaches, dispatch parallel research before answering:

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn search for [USER_QUERY]. [GOAL]: Find relevant internal learnings and adjacent project context. [REQUEST]: Search learnings entries, referenced files, and nearby project history for matches, variants, and contradictory evidence.")
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn search for [USER_QUERY]. [GOAL]: Find external best practices related to the discovered patterns. [REQUEST]: Look up authoritative guidance, common industry patterns, and compare them with the internal learnings you were given.")
```

Present internal learnings first, then clearly separate any external best-practice context from Librarian.
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
~/.opencode/skills/ohmystack/bin/ohmystack-learnings-search --limit 100 2>/dev/null
```
...
**OMO multi-agent dispatch:** Use Explore agents to split the pruning pass into independent checks.

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn prune. [GOAL]: Find stale learnings. [REQUEST]: Search learning entries with files references, verify whether referenced files/paths still exist, and list entries that appear stale.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn prune. [GOAL]: Find contradictory learnings. [REQUEST]: Search for duplicate keys, conflicting insights, confidence drift, and entries superseded by newer project state.")
```
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
~/.opencode/skills/ohmystack/bin/ohmystack-learnings-search --limit 50 2>/dev/null
```
...
If the export is intended to influence process or architecture decisions, enrich it with an external comparison pass:

```typescript
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT]: /learn export. [GOAL]: Compare internal learnings to external best practices. [REQUEST]: For the main exported patterns and pitfalls, identify relevant industry guidance, note where the team's learnings align or diverge, and surface any missing best-practice caveats.")
```

Keep exported internal learnings and external best-practice notes in separate sections.
...
LEARN_FILE="$OHMYSTACK_HOME/projects/$SLUG/learnings.jsonl"
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
