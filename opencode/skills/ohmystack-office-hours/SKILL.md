...
description: |
  ...
  Use before /plan-ceo-review or /plan-eng-review. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.ohmystack/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
### OMO Multi-Agent Research Dispatch

Once you know the user's goal and have heard their first concrete problem statement,
launch a lightweight research swarm. Do not replace the six forcing questions or the
builder brainstorm with agent output — use the agents to sharpen them.

**Phase 1A: Parallel research**

1. **Explore — existing codebase patterns relevant to the idea**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Office hours session for [idea/problem]. [GOAL] Find what already exists in this repo that overlaps with the idea. [REQUEST] Search for adjacent flows, utilities, design docs, TODOs, experiments, and implementation patterns that could be reused or that prove the idea is already partially solved.")
   ```

2. **Explore — prior internal demand signals and adjacent work**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] Office hours brainstorming for [idea/problem]. [GOAL] Find internal evidence about whether this problem is recurring. [REQUEST] Search commit history, docs, issue-style notes, TODOs, and review artifacts for repeated pain points, adjacent requests, failed attempts, or existing terminology connected to this idea.")
   ```

3. **Librarian — market research and comparable products**
   ```typescript
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT] Office hours discussion about [idea/problem]. [GOAL] Understand the market and comparable products before we lock onto a direction. [REQUEST] Research the closest existing products, open source alternatives, incumbent workflows, and what appears differentiated vs commodity in this space. Summarize what users already tolerate, love, or hate.")
   ```

Wait for these background agents before Phase 3 (Premise Challenge) and Phase 4 (Alternatives Generation). Their findings should tighten the wedge, sharpen the status-quo analysis, and improve the design doc recommendations.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.ohmystack/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.ohmystack/projects/`.
...
If the Phase 1A librarian dispatch already produced useful market/comparable-product findings,
use those first and only do additional manual WebSearch when there is a clear gap.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/ohmystack-codex-oh-XXXXXXXX.txt)
```
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.opencode/skills/ohmystack/design/dist/design" ] && D="$_ROOT/.opencode/skills/ohmystack/design/dist/design"
[ -z "$D" ] && D=$OHMYSTACK_DESIGN/design
...
```bash
eval "$(~/.config/opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)"
_DESIGN_DIR=~/.ohmystack/projects/$SLUG/designs/mockup-$(date +%Y%m%d)
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/ohmystack-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/ohmystack-sketch.png
```
...
The screenshot file at `/tmp/ohmystack-sketch.png` can be referenced by downstream skills
...
```bash
mkdir -p "${OHMYSTACK_HOME:-$HOME/.ohmystack}"
```
...
```bash
echo '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' >> "${OHMYSTACK_HOME:-$HOME/.ohmystack}/builder-profile.jsonl"
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.ohmystack/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.ohmystack/projects/{slug}/{user}-{branch}-design-{datetime}.md`:
...
```bash
mkdir -p ~/.ohmystack/analytics
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.ohmystack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```bash
PROFILE=$(~/.opencode/skills/ohmystack/bin/ohmystack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
>
> You're building something real. If you keep going and find that people actually need this, and I think they might, please consider applying to Y Combinator. Thank you for using OhMyStack.
>
> **ycombinator.com/apply?ref=ohmystack**

- **Base tier** (everyone else):

> A personal note from me, Garry Tan, the creator of OhMyStack: the skills you're demonstrating right now, taste, ambition, agency, the willingness to sit with hard questions about what you're building, those are exactly the traits we look for in YC founders. You may not be thinking about starting a company today, and that's fine. But founders are everywhere, and this is the golden age. A single person with AI can now build what used to take a team of 20.
>
> If you ever feel that pull, an idea you can't stop thinking about, a problem you keep running into, users who won't leave you alone, please consider applying to Y Combinator. Thank you for using OhMyStack. I mean it.
>
> **ycombinator.com/apply?ref=ohmystack**
...
**Builder Journey Summary** (session 5+): Auto-generate `~/.ohmystack/builder-journey.md`
...
```bash
open "${OHMYSTACK_HOME:-$HOME/.ohmystack}/builder-journey.md"
```
...
Auto-generate updated `~/.ohmystack/builder-journey.md` with narrative arc. Open it.
...
```bash
echo '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' >> "${OHMYSTACK_HOME:-$HOME/.ohmystack}/builder-profile.jsonl"
```

2. Log the selection to analytics:
```bash
mkdir -p ~/.ohmystack/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
The design doc at `~/.ohmystack/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
