...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /qa-only. (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
| Mode | full | `--regression .ohmystack/qa-reports/baseline.json` |
| Output dir | `.ohmystack/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .ohmystack/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.ohmystack/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)"
   ls -t ~/.ohmystack/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
### Multi-Agent Dispatch: Test Surface Mapping

Before Phase 1 starts, dispatch background mapping agents so the browse-based QA pass has a complete surface map instead of guessing from the landing page alone:

```python
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="TASK: Map the app's test surfaces. Find all user-facing routes/pages, auth gates, nav entry points, forms, interactive widgets, and primary state-changing flows. Return a concise route-by-route checklist with file/path evidence.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="TASK: Enumerate forms and interactive elements worth QA coverage. Find submissions, dialogs, drawers, dropdowns, uploaders, keyboard shortcuts, pagination, filters, and destructive actions. Return repro-oriented coverage notes with selectors or labels when available.")
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="TASK: Gather prior QA context from local reports, test plans, and learnings. Summarize known flaky areas, previously broken flows, and high-risk surfaces to prioritize during this QA run.")
```

Use those agent results to drive the browse test plan. The agents enhance coverage selection; they do NOT replace the real browse-based QA workflow.
...
If the root cause is still unclear after two focused fix attempts, stop guessing and dispatch Oracle before attempt three:

```python
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="TASK: Root-cause consultation for QA bug. BUG: [bug]. FILES ALREADY CHECKED: [paths]. FAILED FIX ATTEMPTS: [1..2 summary]. BROWSE EVIDENCE: [console/screenshot/snapshot]. Return the most likely root cause, risky assumptions, and the narrowest next fix to try.")
```

Use Oracle to sharpen the next attempt; continue the same atomic fix → commit → re-verify loop after the consultation.
...
For each bug found, dispatch a dedicated fix agent instead of batching fixes in the main thread:

```python
task(category="quick", load_skills=[], run_in_background=false, prompt="TASK: Fix [bug]. FILE: [path]. ROOT CAUSE: [cause]. FIX: [specific change]. MUST: commit atomically, re-verify with browse.")
```

This dispatch is per-bug, per-commit. One bug in, one fix commit out, then re-verify before moving to the next issue.
...
- Include full attribution comment:
  ...
  // Report: .ohmystack/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.ohmystack/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:** Write test outcome artifact for cross-session context:
```bash
eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
```
Write to `~/.ohmystack/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
