...
description: |
  ...
  or when the plan feels like it could be thinking bigger. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
SLUG=$(~/.opencode/skills/ohmystack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.ohmystack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
Replace generic "research the market first" behavior with explicit sub-agent dispatch.
Keep the bash-based system audit above. Add this agent swarm immediately after you
understand the plan/problem statement and before Step 0A starts.

**Phase 1: Research (parallel)**
Launch background agents in parallel:

1. **Explore — internal leverage + existing product surface**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] CEO/founder plan review for [feature/topic]. [GOAL] Find existing codepaths, product surfaces, TODOs, and prior work that already touch this problem. [REQUEST] Search the repo for relevant flows, user-facing surfaces, docs, and known pain points. Return concrete file paths, patterns worth reusing, and anti-patterns to avoid.")
   ```

2. **Explore — competitor clues already embedded in the repo**
   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT] CEO review for [feature/topic]. [GOAL] Find any references in the codebase/docs to competitors, incumbent workflows, previous product bets, or market assumptions. [REQUEST] Search plans, docs, TODOs, and review artifacts for competitor names, user requests, prior rejected ideas, and strategic constraints.")
   ```

3. **Librarian — industry benchmarks + comparable products**
   ```typescript
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT] Reviewing a product plan for [feature/topic]. [GOAL] Find current industry benchmarks and comparable products. [REQUEST] Research the product category landscape, notable incumbents, benchmark UX/business patterns, and 3-5 comparable products with what they do well, where they fail, and what users appear to value most.")
   ```

4. **Librarian — search-before-building synthesis**
   ```typescript
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT] CEO plan review for [feature/topic]. [GOAL] Pressure-test the conventional wisdom in this space. [REQUEST] Gather the strongest arguments for the default industry approach, major alternatives, and any contrarian patterns that appear to be working in practice. Summarize as tried-and-true vs emerging vs suspect.")
   ```

Wait for the background agents to complete before proceeding to Step 0. Use their findings to sharpen Premise Challenge, Dream State Mapping, and expansion opportunities.

**Phase 2: Strategic verification (sync)**
After the background findings are in, run a synchronous oracle consultation before final mode framing:

```typescript
task(category="deep", load_skills=[], run_in_background=false, prompt="[CONTEXT] CEO/founder review for [feature/topic]. Background research has surfaced current codebase leverage, market benchmarks, and comparable products. [GOAL] Verify the strategic scope posture before I guide the user. [REQUEST] Evaluate whether SCOPE EXPANSION, SELECTIVE EXPANSION, HOLD SCOPE, or SCOPE REDUCTION best fits the evidence. Call out the strongest strategic risk, the strongest upside, and which scope decisions require explicit user opt-in.")
```
...
```bash
eval "$(~/.opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.ohmystack/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.ohmystack/projects/$SLUG/ceo-plans/{old-plan}.md ~/.ohmystack/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.ohmystack/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```bash
mkdir -p ~/.ohmystack/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.ohmystack/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + ohmystack,
...
**Persist the result:**
```bash
~/.config/opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+ohmystack: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)"
rm -f ~/.ohmystack/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`~/.ohmystack/` (user config directory, not project files). The skill preamble
already writes to `~/.ohmystack/sessions/` and `~/.ohmystack/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- Search the plan file for a \`## OHMYSTACK REVIEW REPORT\` section **anywhere** in the file
  (not just at the end — content may have been added after it).
- If found, **replace it** entirely using the Edit tool. Match from \`## OHMYSTACK REVIEW REPORT\`
...
- **B)** Keep in `~/.ohmystack/projects/` only (local, personal reference)
- **C)** Skip
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
