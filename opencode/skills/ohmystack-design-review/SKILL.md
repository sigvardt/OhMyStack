...
description: |
  ...
  wants to polish the look of a live site. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
**Find the ohmystack designer (optional — enables target mockup generation):**
...
REPORT_DIR=~/.ohmystack/projects/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
**Local:** `.ohmystack/design-reports/design-audit-{domain}-{YYYY-MM-DD}.md`

**Project-scoped:**
```bash
eval "$(~/.config/opencode/skills/ohmystack/bin/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
```
Write to: `~/.ohmystack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md`
...
### OMO multi-agent dispatch during baseline

Dispatch specialists as soon as the baseline scope is known. They do the heavy research while you run the screenshot workflow:

1. **Explore pattern scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Inspect the repo for existing design patterns, design tokens, reusable components, spacing rules, typography choices, color variables, and page patterns relevant to the audited screens. Return concrete files, token names, and reuse opportunities so fixes stay consistent with the codebase.")
   ```

2. **Momus adversarial design critique**
   ```
   task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="Critique the audited UI like an adversarial design reviewer. Hunt for hierarchy failures, broken affordances, uncanny/AI-slop patterns, visual inconsistency, accessibility-hostile choices, and interaction regressions that would embarrass the product team. Return only actionable findings with rationale.")
   ```

3. **Sisyphus-Junior fix-loop executor**
   ```
   task(category="quick", load_skills=["design-review"], run_in_background=false, prompt="For a single approved finding, execute one iteration of the fix loop only: locate the source, make the minimum targeted change, capture before/after evidence, and verify whether the issue is resolved without broad refactoring.")
   ```

Use the first two dispatches for research/critique and the quick dispatch pattern repeatedly inside Phase 8 for each finding you choose to fix.
...
```
~/.ohmystack/projects/$SLUG/designs/design-audit-{YYYYMMDD}/
...
Before changing code, consume the Explore results for matching tokens/components so your fix reuses existing patterns instead of inventing new ones.

### 8a.5. Target Mockup (if DESIGN_READY)

If the ohmystack designer is available and the finding involves visual layout, hierarchy, or spacing (not just a CSS value fix like wrong color or font-size), generate a target mockup showing what the corrected version should look like:
...
**Dispatch pattern for every fixable finding:** after source is located and before commit, you may invoke the quick Sisyphus-Junior executor for that single finding to keep the loop tight:

```
task(category="quick", load_skills=["design-review"], run_in_background=false, prompt="Fix FINDING-NNN only. Use the existing design tokens/patterns already discovered, make the minimum source change, then report exactly what changed and what still needs manual verification.")
```
...
Cross-check the before/after pair against Momus's critique and the Explore pattern scan. If the fix resolves the screenshot issue but introduces a new inconsistency against established tokens/patterns, keep iterating before committing.
...
```
Write a one-line summary to `~/.ohmystack/projects/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
