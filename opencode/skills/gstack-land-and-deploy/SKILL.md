...
```bash
~/.config/opencode/skills/ohmystack/bin/gstack-review-read 2>/dev/null
```
...
```bash
cat ~/.config/opencode/skills/ohmystack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(~/.config/opencode/skills/ohmystack/bin/gstack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
