...
description: |
  ...
  Use when: "security audit", "threat model", "pentest review", "OWASP", "CSO review". (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"cso","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"cso","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
**OMO multi-agent dispatch:** Run the high-signal scans in parallel so secrets work does not bottleneck the audit.

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 2 secrets archaeology. [GOAL]: Find exposed credentials in git history. [REQUEST]: Search for known secret prefixes, suspicious additions in config/env files, and branch-only leaks when diff mode is active.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 2 secrets archaeology. [GOAL]: Find tracked secret files. [REQUEST]: Search for tracked .env files, weak .gitignore coverage, suspicious example values, and secret-bearing config files.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 2 secrets archaeology. [GOAL]: Find CI secret exposure. [REQUEST]: Search workflow files and CI configs for inline credentials, unsafe env propagation, and log-leak patterns.")
```

Synthesize the results into one candidate set, then apply the Phase 12 confidence gate before reporting anything.
...
**OMO multi-agent dispatch:** Split supply-chain work by evidence source.

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 3 dependency audit. [GOAL]: Find vulnerable or risky dependencies. [REQUEST]: Inspect lockfiles, manifests, audit outputs, and direct dependency lists for high-signal risk.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 3 dependency audit. [GOAL]: Find install-script and provenance risks. [REQUEST]: Search production dependencies for preinstall/postinstall/install scripts, untrusted registries, and missing lockfile integrity signals.")
```

If a dependency issue is unclear or tied to an ecosystem convention you do not recognize, queue a documentation lookup with Librarian before scoring severity.

```typescript
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 3 dependency audit. [GOAL]: Validate ecosystem-specific supply-chain risk. [REQUEST]: Look up the package manager or dependency pattern involved, summarize known abuse cases or guidance, and cite the best-practice baseline for severity calibration.")
```
...
**OMO multi-agent dispatch:** Treat CI/CD as its own attack surface and scan it in parallel.

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 4 CI/CD audit. [GOAL]: Find dangerous workflow triggers and checkout patterns. [REQUEST]: Search workflow files for pull_request_target, checkout of PR refs, and untrusted-input execution paths.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 4 CI/CD audit. [GOAL]: Find unpinned or over-privileged actions. [REQUEST]: Search for third-party actions without SHA pinning, broad permissions, unsafe secrets exposure, and missing CODEOWNERS protection.")
```
...
**FP rules:** ohmystack's own skills are trusted (check if skill path resolves to a known repo). Skills that use `curl` for legitimate purposes (downloading tools, health checks) need context — only flag when the target URL is suspicious or when the command includes credential variables.
...
Before finalizing category findings, use Librarian as the external reference check for OWASP and CWE alignment.

```typescript
task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 9 OWASP assessment. [GOAL]: Calibrate findings against OWASP Top 10 and CWE references. [REQUEST]: For the candidate issues found, map them to current OWASP Top 10 categories, likely CWE entries, and the baseline exploit expectations for severity/confidence scoring.")
```
...
Use Oracle to pressure-test your STRIDE reasoning before publishing the model.

```typescript
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="[CONTEXT]: CSO Phase 10 STRIDE analysis. Architecture summary: [summary]. Candidate findings: [findings]. [GOAL]: Pressure-test the threat model. [REQUEST]: Identify the highest-risk spoofing, tampering, repudiation, information disclosure, denial-of-service, and elevation-of-privilege paths; challenge weak assumptions; and highlight missing trust boundaries.")
```

Oracle is the skeptic, not the source of truth. Keep the existing confidence gates: daily mode still requires 8/10 confidence, comprehensive mode still uses the 2/10 bar.
...
22. Skill files that are part of ohmystack itself (trusted source)
...
**Trend Tracking:** If prior reports exist in `.ohmystack/security-reports/`:
```
...
```bash
mkdir -p .ohmystack/security-reports
```

Write findings to `.ohmystack/security-reports/{date}-{HHMMSS}.json` using this schema:
...
If `.ohmystack/` is not in `.gitignore`, note it in findings — security reports should stay local.
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
