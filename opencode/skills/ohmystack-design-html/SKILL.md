...
description: |
  ...
  Proactively suggest when user has approved a design or has a plan ready. (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-html","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"design-html","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
_CEO=$(ls -t ~/.ohmystack/projects/$SLUG/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t ~/.ohmystack/projects/$SLUG/designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t ~/.ohmystack/projects/$SLUG/designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t ~/.ohmystack/projects/$SLUG/designs/*/finalized.html 2>/dev/null | head -1)
...
Once the mode is known, dispatch specialists so implementation uses both project patterns and Pretext-specific guidance:

1. **Explore component-pattern scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Inspect the repo for existing frontend component patterns, layout shells, token usage, accessibility conventions, and any reusable HTML/CSS/TSX structures relevant to the screen being implemented. Return concrete files and patterns to mirror.")
   ```

2. **Librarian Pretext reference scan**
   ```
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Research the relevant Pretext APIs and implementation patterns for this design type. Focus on which APIs best fit the requested layout, how to preserve dynamic text reflow, and how to stay within the intended lightweight footprint.")
   ```

3. **Oracle implementation verifier**
   ```
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="After the implementation spec and chosen Pretext tier are drafted, verify that the proposed HTML/CSS architecture is coherent, matches the design context, preserves dynamic layout behavior, and does not quietly violate the 30KB-overhead / zero-deps intent.")
   ```

The main skill still chooses the smart routing tier and writes the final artifact. Specialists provide the surrounding evidence.
...
Before finalizing the implementation spec, merge in:
- the **Explore** scan for existing component/tokens you should reuse,
- the **Librarian** Pretext guidance for API and pattern fit,
- the **Oracle** verifier's notes on coherence or footprint risk.
...
If the Librarian dispatch recommends a different API tier than your first instinct, resolve the disagreement explicitly and choose the option that best preserves dynamic layouts while staying lean.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/.opencode/skills/ohmystack/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/.opencode/skills/ohmystack/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f ~/.opencode/skills/ohmystack/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=~/.opencode/skills/ohmystack/design-html/vendor/pretext.js
...
`~/.ohmystack/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`~/.ohmystack/projects/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`

When writing the file, reuse the closest matching project patterns discovered by Explore unless they directly conflict with the approved design context. If Oracle flags that the generated structure is drifting away from the approved mockup/plan, adjust before moving on.
...
$B screenshot /tmp/ohmystack-verify-mobile.png --width 375
$B screenshot /tmp/ohmystack-verify-tablet.png --width 768
$B screenshot /tmp/ohmystack-verify-desktop.png --width 1440
```
