...
description: |
  ...
  Proactively suggest when the user is about to merge or land code changes. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
~/.config/opencode/skills/ohmystack/bin/ohmystack-learnings-log '{
  "type": "pitfall",
...
### Step 4.1: OMO multi-agent dispatch

Before writing findings, dispatch background specialists so the main skill can orchestrate one unified verdict:

1. **Oracle structural review** — launch:
   ```
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Review the diff against origin/<base>. Focus on SQL safety, LLM trust boundaries, conditional side effects, race conditions, shell injection, enum/value completeness, and any structural correctness issues that tests may miss. Return findings with severity, file:line evidence, and an APPROVE/REJECT recommendation.")
   ```

2. **Momus adversarial critique** — launch:
   ```
   task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="Adversarially critique this branch diff against origin/<base>. Assume the implementation is hiding brittle assumptions. Try to break the plan in production: edge cases, abuse cases, rollback hazards, bad incentives, silent corruption, and coordination failures. Return only concrete risks with evidence plus a ship/no-ship recommendation.")
   ```

3. **Explore impact scan** — launch:
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Scan the repo for code outside the diff that could be affected by these changes. Prioritize sibling enum handlers, auth/trust-boundary code, SQL/query call sites, side-effectful jobs, and other consumers of touched interfaces. Return the most relevant files, why they matter, and any missed handling paths.")
   ```

4. Continue your own diff review while the agents run. Do **not** wait idly and do **not** duplicate their searches. When all three complete, merge their evidence into the normal checklist flow.

### Step 4.2: Unified verdict synthesis

Produce one final verdict for the branch using **all** review inputs: your checklist review, Oracle, Momus, Explore, and Greptile classifications if present.

- **APPROVE** only if no unresolved critical issues remain and no dispatched specialist returns a substantiated REJECT.
- **REJECT** if any unresolved critical issue remains, or if Oracle/Momus surfaces a concrete ship blocker you cannot clear with evidence.
- If specialists disagree, show the disagreement explicitly, resolve it with repo evidence, and then issue one final branch verdict.

Output header format:

```
VERDICT: APPROVE | REJECT
STRUCTURAL REVIEW: <summary of Oracle>
ADVERSARIAL REVIEW: <summary of Momus>
IMPACT SCAN: <summary of Explore>
```
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"COMMIT"}'
```
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
