...
description: |
  ...
  yesterday", or is troubleshooting why something stopped working. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"investigate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"investigate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
5. **OMO multi-agent evidence sweep:** Before settling on a root-cause hypothesis, launch **3-5 parallel Explore agents** that investigate the failure from different angles. Keep prompts narrowly scoped so each agent returns distinct evidence.

   ```typescript
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: Debugging [error]. [GOAL]: Find error origin. [REQUEST]: Search for error message, stack trace origins, recent changes to affected files, and the earliest frame that points to application code.")
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: Debugging [error]. [GOAL]: Find related code paths. [REQUEST]: Search for callers, dependencies, and upstream/downstream code paths of the affected function or module.")
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: Debugging [error]. [GOAL]: Find environment/config contributors. [REQUEST]: Search configs, feature flags, env handling, deployment differences, and initialization paths that could explain the failure.")
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="[CONTEXT]: Debugging [error]. [GOAL]: Find failing verification signals. [REQUEST]: Search tests, flaky specs, recent CI failures, reproduction scripts, and known issue notes related to this behavior.")
   ```

   Add a fifth Explore agent when needed for data migrations, cache behavior, concurrency, or external integrations. Wait for the evidence, synthesize overlaps, and only then write the root cause hypothesis.
...
```bash
STATE_DIR="${CLAUDE_PLUGIN_DATA:-$HOME/.ohmystack}"
mkdir -p "$STATE_DIR"
...
1. **Confirm the hypothesis:** Add a temporary log statement, assertion, or debug output at the suspected root cause. Run the reproduction. Does the evidence match?
   ...
   **Oracle cross-check (recommended before committing to the fix):**
   ```typescript
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="[CONTEXT]: Debugging [error]. Evidence: [evidence]. [GOAL]: Evaluate hypotheses. [REQUEST]: Which hypothesis best explains the evidence, what evidence is still missing, and what observation would falsify the leading theory?")
   ```

   Use Oracle to challenge your reasoning, not replace it. If Oracle identifies a better explanation, return to Phase 1 and gather the missing proof.
...
3. **Write a regression test** that:
   ...
   **Implementation dispatch:** Once the root cause is confirmed and the scope lock is in place, hand the concrete code change to Sisyphus-Junior for execution.
   ```typescript
   task(category="quick", load_skills=[], run_in_background=false, prompt="Fix: [root cause]. File: [path]. Change: [specific fix]. Add or update the regression test that proves the bug is fixed.")
   ```

   The dispatch prompt must include the confirmed root cause, target file(s), exact behavior to change, and the regression test requirement. Never dispatch a speculative fix.
...
```bash
$OHMYSTACK_BIN/ohmystack-learnings-log '{"skill":"investigate","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
