...
description: |
  ...
  "let another agent use my browser", or "give browser access". (ohmystack)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"pair-agent","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"pair-agent","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
Your ohmystack browser runs a local HTTP server. This skill creates a one-time setup key,
...
side panel if you have OhMyStack Browser open."
...
credentials are written to `~/.openclaw/skills/ohmystack/browse-remote.json`.
...
to `~/.codex/skills/ohmystack/browse-remote.json`.
...
`~/.cursor/skills/ohmystack/browse-remote.json`.
