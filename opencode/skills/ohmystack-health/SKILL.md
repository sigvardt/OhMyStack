...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"health","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"health","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
If the project's tools are safe to run in parallel, prefer OMO dispatch so the main skill can orchestrate the dashboard while specialists run the checks. Use sequential fallback only when tools contend for the same cache, DB, or lock files.

### Step 2.1: OMO multi-agent dispatch

Dispatch one background explorer per major check that exists in the detected health stack:

1. **Type checker / linter scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Run the detected type checker and linter for this repo. Capture exit codes, duration, concise raw output, and countable error/warning totals suitable for the /health scoring rubric.")
   ```

2. **Tests / dead code scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Run the detected test command and dead-code tool for this repo. Capture exit codes, duration, pass/fail counts, and unused symbol counts suitable for the /health scoring rubric.")
   ```

3. **Shell hygiene / score verifier**
   ```
   task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Review the collected /health tool results after they land. Verify category scoring, skipped-tool weight redistribution, composite-score math, and identify the highest-risk regressions to highlight in the dashboard.")
   ```

4. **Optional shellcheck explorer**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="If shell scripts exist, run the detected shell lint command, capture distinct findings, exit code, duration, and the most actionable output lines for the /health report. If no shell lint applies, return SKIPPED with reason.")
   ```

Collect all results before final scoring. Do **not** duplicate a specialist's check in the main thread unless you are explicitly falling back from a failed dispatch.
...
### Step 3.1: Composite score verification

After you compute the first-pass category scores, compare them against the Oracle verifier dispatch:

- If Oracle agrees, use the score as final.
- If Oracle disputes the math or a category count, resolve the discrepancy with the actual tool output and state which interpretation won.
- The final composite score must still use the exact weights above and remain on a 0-10 scale.
...
Append one JSONL line to `~/.ohmystack/projects/$SLUG/health-history.jsonl`:
...
Read the last 10 entries from `~/.ohmystack/projects/$SLUG/health-history.jsonl` (if the
file exists and has prior entries).

```bash
eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)" && mkdir -p ~/.ohmystack/projects/$SLUG
tail -10 ~/.ohmystack/projects/$SLUG/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
