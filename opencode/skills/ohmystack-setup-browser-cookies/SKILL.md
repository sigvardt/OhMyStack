...
description: |
  ...
  "login to the site", or "authenticate the browser". (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-browser-cookies","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"setup-browser-cookies","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
