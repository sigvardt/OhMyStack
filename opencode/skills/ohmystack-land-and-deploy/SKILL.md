...
description: |
  ...
  "land it", "ship it to production". (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
```bash
eval "$($OHMYSTACK_BIN/ohmystack-slug 2>/dev/null)"
if [ ! -f ~/.ohmystack/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.ohmystack/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
~/.opencode/skills/ohmystack/bin/ohmystack-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.ohmystack/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.ohmystack/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
Before synthesizing the readiness report, launch the parallel production-risk gate on the exact PR details and current diff summary:

```typescript
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Final pre-merge review: [PR details]. Check for deployment risks.")
task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="Pre-merge critique: [PR details]. Find risks that would manifest in production.")
```

Wait for both before offering the final merge decision. Both must approve before merge. If either surfaces a blocking deployment risk, treat it as a blocker in the readiness gate and do not merge until resolved.
...
```bash
cat ~/.opencode/skills/ohmystack/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
ls -t ~/.ohmystack-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.ohmystack-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `ohmystack-diff-scope` to classify the changes:

```bash
eval $(~/.opencode/skills/ohmystack/bin/ohmystack-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".ohmystack/deploy-reports/post-deploy.png"
```
...
In parallel with the main canary check, launch background explore monitors for independent health dimensions so you are not relying on a single foreground pass:

```typescript
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Post-deploy health monitoring for [URL]. Watch primary health checks, uptime signals, and page availability while /land-and-deploy performs its main verification. Report anomalies with timestamps.")
task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Post-deploy health monitoring for [URL]. Watch console/performance signals and compare them against the deploy verification baseline. Report anomalies with severity and likely blast radius.")
```
...
If the foreground canary or either explore monitor detects suspicious behavior, dispatch Oracle to analyze whether the anomaly is transient, environmental, or a real deployment regression:

```typescript
task(subagent_type="oracle", run_in_background=true, load_skills=[], prompt="Analyze these post-deploy anomalies for [URL]: [symptoms/evidence]. Determine whether this is a real deployment regression, likely transient warm-up, or an external dependency issue. Recommend whether to continue, investigate, or revert.")
```

Use that Oracle verdict to sharpen the recommendation in the rollback AskUserQuestion.
...
```bash
mkdir -p .ohmystack/deploy-reports
```
...
Save report to `.ohmystack/deploy-reports/{date}-pr{number}-deploy.md`.
