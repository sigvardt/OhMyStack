...
description: |
  ...
  what it could look like. (ohmystack)
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$OHMYSTACK_BIN/ohmystack-timeline-log '{"skill":"design-shotgun","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
...
_PREV=$(find ~/.ohmystack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
### OMO multi-agent dispatch during context + generation

Once you know the screen and audience, dispatch supporting specialists so the variant generator has stronger inputs:

1. **Librarian inspiration scan**
   ```
   task(subagent_type="librarian", run_in_background=true, load_skills=[], prompt="Research relevant design inspiration and current visual trends for this product category and screen type. Return references, recurring motifs, overused clichés, and a few fresh directions worth exploring.")
   ```

2. **Explore token + pattern scan**
   ```
   task(subagent_type="explore", run_in_background=true, load_skills=[], prompt="Inspect the project for existing design tokens, component patterns, page shells, layout conventions, and prior approved designs that should inform new variants. Return concrete file paths, token names, and any constraints the new variants should respect.")
   ```

3. **Momus differentiation critique**
   ```
   task(subagent_type="momus", run_in_background=true, load_skills=[], prompt="Critique the emerging variant concepts adversarially. Flag concepts that are too generic, trend-chasing, inconsistent with the product, or likely to produce AI-slop. Push the concepts toward clearer differentiation and more intentional tradeoffs.")
   ```

The main skill still owns the comparison board, structured feedback loop, and iteration decisions.
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find ~/.ohmystack/projects/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
Blend the taste-memory summary with the Explore scan so existing project tokens/patterns and previously approved taste pull in the same direction.
...
Also fold in the Librarian inspiration scan and Momus critique so each concept is both grounded and meaningfully differentiated.
...
Before launching the variant generators, do a final brief pass:
- Use **Librarian** results to enrich references and trend awareness.
- Use **Explore** results to inject real project tokens/components into the briefs.
- Use **Momus** results to remove concepts that feel generic, contradictory, or too close together.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output ~/.ohmystack/...`
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `~/.ohmystack/projects/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
