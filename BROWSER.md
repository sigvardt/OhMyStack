...
This document covers the command reference and internals of ohmystack's headless browser.
...
ohmystack's browser is a compiled CLI binary that talks to a persistent local Chromium daemon over HTTP. The CLI is a thin client — it reads a state file, sends a command, and prints the response to stdout. The server does the real work via [Playwright](https://playwright.dev/).
...
Each server session generates a random UUID as a bearer token. The token is written to the state file (`.ohmystack/browse.json`) with chmod 600. Every HTTP request must include `Authorization: Bearer <token>`. This prevents other processes on the machine from controlling the browser.
...
- Console: `.ohmystack/browse-console.log`
- Network: `.ohmystack/browse-network.log`
- Dialog: `.ohmystack/browse-dialog.log`
...
The window has a subtle green shimmer line at the top edge and a floating "ohmystack" pill in the bottom-right corner so you always know which Chrome window is being controlled.
...
# Click the ohmystack icon in toolbar → Open Side Panel
```
...
```bash
bin/ohmystack-extension    # opens chrome://extensions, copies path to clipboard
```
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.claude/skills/ohmystack/extension`
   - Dev/source: `<ohmystack-repo>/extension`
...
5. **Pin it:** Click the puzzle piece icon (Extensions) in the toolbar → pin "ohmystack browse"
6. **Set the port:** Click the ohmystack icon → enter the port from `$B status` or `.ohmystack/browse.json`
7. **Open Side Panel:** Click the ohmystack icon → "Open Side Panel"
...
| **Connection pill** | Small "ohmystack" pill in the bottom-right corner of every page when connected |
...
Each workspace gets its own isolated browser instance with its own Chromium process, tabs, cookies, and logs. State is stored in `.ohmystack/` inside the project root (detected via `git rev-parse --show-toplevel`).

| Workspace | State file | Port |
|-----------|------------|------|
| `/code/project-a` | `/code/project-a/.ohmystack/browse.json` | random (10000-60000) |
| `/code/project-b` | `/code/project-b/.ohmystack/browse.json` | random (10000-60000) |
...
| `BROWSE_STATE_FILE` | `.ohmystack/browse.json` | Path to state file (CLI passes to server) |
...
| **ohmystack browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |

The context overhead difference compounds fast. In a 20-command browser session, MCP tools burn 30,000-40,000 tokens on protocol framing alone. ohmystack burns zero.
...
ohmystack skips all of this. Compiled binary. Plain text in, plain text out. No protocol. No schema. No connection management.
...
| File | Role |
|------|------|
| `browse/src/cli.ts` | Entry point. Reads `.ohmystack/browse.json`, sends HTTP to the server, prints response. |
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.claude/skills/ohmystack && git pull`
3. Rebuild: `cd ~/.claude/skills/ohmystack && bun run build`

Or copy the binary directly: `cp browse/dist/browse ~/.claude/skills/ohmystack/browse/dist/browse`
