 ...
 *
 * Telemetry data flow:
 *   1. Always: local JSONL append to ~/.ohmystack/analytics/ (inline, inspectable)
 *   2. Remote telemetry calls are disabled in OhMyStack
 */
...
OHMYSTACK_ROOT="$HOME/${hostConfig.globalRoot}"
[ -n "$_ROOT" ] && [ -d "$_ROOT/${ctx.paths.localSkillRoot}" ] && OHMYSTACK_ROOT="$_ROOT/${ctx.paths.localSkillRoot}"
...
\`\`\`bash
${runtimeRoot}_UPD=$(${ctx.paths.binDir}/ohmystack-update-check 2>/dev/null || ${ctx.paths.localSkillRoot}/bin/ohmystack-update-check 2>/dev/null || true)
...
_PROACTIVE=$(${ctx.paths.binDir}/ohmystack-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f ~/.ohmystack/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$(${ctx.paths.binDir}/ohmystack-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <(${ctx.paths.binDir}/ohmystack-repo-mode 2>/dev/null) || true
REPO_MODE=\${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f ~/.ohmystack/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$(${ctx.paths.binDir}/ohmystack-config get telemetry 2>/dev/null || true)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"${ctx.skillName}","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.ohmystack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
# Learnings count
eval "$(${ctx.paths.binDir}/ohmystack-slug 2>/dev/null)" 2>/dev/null || true
_LEARN_FILE="\${OHMYSTACK_HOME:-$HOME/.ohmystack}/projects/\${SLUG:-unknown}/learnings.jsonl"
if [ -f "$_LEARN_FILE" ]; then
  _LEARN_COUNT=$(wc -l < "$_LEARN_FILE" 2>/dev/null | tr -d ' ')
  echo "LEARNINGS: $_LEARN_COUNT entries loaded"
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ${ctx.paths.binDir}/ohmystack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
# Session timeline: record skill start (local-only, never sent anywhere)
${ctx.paths.binDir}/ohmystack-timeline-log '{"skill":"${ctx.skillName}","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
# Check if CLAUDE.md has routing rules
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$(${ctx.paths.binDir}/ohmystack-config get routing_declined 2>/dev/null || echo "false")
...
function generateUpgradeCheck(ctx: TemplateContext): string {
  return `If \`PROACTIVE\` is \`"false"\`, do not proactively suggest OhMyStack skills AND do not
...
If output shows \`UPGRADE_AVAILABLE <old> <new>\`: read \`${ctx.paths.skillRoot}/ohmystack-upgrade/SKILL.md\` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If \`JUST_UPGRADED <from> <to>\`: tell user "Running OhMyStack v{to} (just updated!)" and continue.`;
}
...
If A: run \`${ctx.paths.binDir}/ohmystack-config set telemetry community\`
...
If B→A: run \`${ctx.paths.binDir}/ohmystack-config set telemetry anonymous\`
If B→B: run \`${ctx.paths.binDir}/ohmystack-config set telemetry off\`
...
If A: run \`${ctx.paths.binDir}/ohmystack-config set proactive true\`
If B: run \`${ctx.paths.binDir}/ohmystack-config set proactive false\`
...
If B: run \`${ctx.paths.binDir}/ohmystack-config set routing_declined true\`
...
function generateVendoringDeprecation(ctx: TemplateContext): string {
  return `If \`VENDORED_OHMYSTACK\` is \`yes\`: This project has a vendored copy of OhMyStack at
...
3. Run \`${ctx.paths.binDir}/ohmystack-team-init required\` (or \`optional\`)
...
\`\`\`bash
${ctx.paths.binDir}/ohmystack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
\`\`\`bash
eval "$(${binDir}/ohmystack-slug 2>/dev/null)"
