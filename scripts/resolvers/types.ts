 ...
 * Non-Claude hosts use $OHMYSTACK_ROOT env vars (set by preamble).
 */
function buildHostPaths(): Record<string, HostPaths> {
  ...
  for (const config of ALL_HOST_CONFIGS) {
    if (config.usesEnvVars) {
      paths[config.name] = {
        skillRoot: '$OHMYSTACK_ROOT',
        localSkillRoot: config.localSkillRoot,
        binDir: '$OHMYSTACK_BIN',
        browseDir: '$OHMYSTACK_BROWSE',
        designDir: '$OHMYSTACK_DESIGN',
      };
    } else {
