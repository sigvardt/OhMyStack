#!/bin/bash
# Build OhMyStack Browser.app — macOS application bundle
#
...
#
# Output: dist/OhMyStack Browser.app and dist/OhMyStack-Browser.dmg
#
# Usage:
...
APP_NAME="OhMyStack Browser"
BUNDLE_ID="com.ohmystack.browser"
...
# Launcher script
cp "$ROOT/scripts/app/ohmystack-browser" "$APP_DIR/Contents/MacOS/ohmystack-browser"
chmod +x "$APP_DIR/Contents/MacOS/ohmystack-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "OhMyStack Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the OhMyStack icon
...
<dict>
  ...
  <string>ohmystack-browser</string>
...
echo "    Contents/MacOS/ohmystack-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/OhMyStack-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"
