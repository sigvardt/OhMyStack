# Design: ohmystack Visual Design Generation (`design` binary)
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.ohmystack/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/ohmystack-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/ohmystack-mockup-{timestamp}.png`
...
```typescript
// claude host:
designDir: '~/.claude/skills/ohmystack/design/dist'
// codex host:
designDir: '$OHMYSTACK_DESIGN'
```
Note: Codex runtime setup (`setup` script) must also export `OHMYSTACK_DESIGN` env var, similar to how `OHMYSTACK_BROWSE` is set.

**`$D` resolution bash block** (generated by `{{DESIGN_SETUP}}`):
```bash
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/ohmystack/design/dist/design" ] && D="$_ROOT/.claude/skills/ohmystack/design/dist/design"
[ -z "$D" ] && D=~/.claude/skills/ohmystack/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `OHMYSTACK_DESIGN` env var export for Codex host |
...
1. Read `~/.ohmystack/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.ohmystack/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/ohmystack/` install path
...
- Auth module (read ~/.ohmystack/openai.json, fallback to env var, guided setup flow)
...
- Add OHMYSTACK_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
