...
ohmystack projects... CLAUDE.md works as-is.
...
### Critical env vars for ohmystack integration
...
Without native `.slate/` publishing, ohmystack skills vanish when this flag is set.
...
ohmystack skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install ohmystack for Claude Code
...
- Host names duplicated in `bin/ohmystack-platform-detect`, `bin/ohmystack-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/ohmystack-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/ohmystack-platform-detect` doesn't detect Slate
