...
When Claude controls your real browser via CDP (ohmystack `$B connect`), you look at two
windows: **Conductor** (to see Claude's thinking) and **Chrome** (to see Claude's actions).

ohmystack's Chrome extension Side Panel shows browse activity — every command, result,
...
A "Session" tab in the ohmystack Chrome extension Side Panel that shows:
  ...
  "workspaces": [
    {
      ...
      "directory": "/Users/garry/ohmystack",
      "pid": 12345,
      "active": true
    }
  ]
...
## What ohmystack builds (extension side)
