# Remote Browser Access — How to Pair With a OhMyStack Browser

A OhMyStack Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
OhMyStack Browser Server                 Any AI agent
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/ohmystack/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/ohmystack/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/ohmystack/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.ohmystack/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.ohmystack/ngrok.env`
