/**
 * ohmystack browse — content script
 *
...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'ohmystack-status-pill';
  ...
  statusPill.innerHTML = `<span class="ohmystack-pill-dot"></span> ohmystack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'ohmystack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'ohmystack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'ohmystack-ref-panel';

  const header = document.createElement('div');
  header.className = 'ohmystack-ref-panel-header';
  header.textContent = `ohmystack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'ohmystack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'ohmystack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'ohmystack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'ohmystack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'ohmystack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'ohmystack-ref-panel-more';
...
// Do NOT dispatch ohmystack-extension-ready here — the extension being loaded
