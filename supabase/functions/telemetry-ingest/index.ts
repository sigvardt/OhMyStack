// ohmystack telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/ohmystack-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.ohmystack_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        ohmystack_version: String(event.ohmystack_version).slice(0, 20),
      ...
      if (event.installation_id) {
        installationUpserts.set(event.installation_id, {
          version: event.ohmystack_version,
          os: event.os,
        });
      }
    }
        ...
        .upsert(
          {
            ...
            ohmystack_version: data.version,
            os: data.os,
          },
